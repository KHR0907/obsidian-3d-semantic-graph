...
- 선택한 노트와 연결된 노트를 보여주는 Inspector 패널
...
4. API 키가 없거나 임베딩 요청이 실패하면 구형 레이아웃으로 대체합니다.
...
5. 노드를 클릭하면 Inspector에 고정되고, `Shift+클릭` 또는 Inspector 버튼으로 노트를 엽니다.

## 설정 항목

| 설정 | 설명 | 기본값 |
| --- | --- | --- |
| API Key | OpenAI API 키. 비워 두면 구형 레이아웃만 사용합니다. | 비어 있음 |
