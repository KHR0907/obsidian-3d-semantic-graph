...
4. API キーがない場合、または埋め込み処理に失敗した場合は決定的な球状レイアウトを使います。
5. 球状レイアウトモードでは、パスのハッシュに基づく安定した順序と golden-angle 配置でノートを 3D 球体内部に分布させるため、再読み込みしても再現可能ですが意味ベースの配置ではありません。
...
5. ノートを直接開くにはノードを `Shift`+クリックします。

## 設定項目

| 設定 | 説明 | 既定値 |
| --- | --- | --- |
| API Key | OpenAI API キー。空の場合は意味埋め込みの代わりに決定的な球状レイアウト fallback を使用 | 空 |
