...
3. 커스텀 벡터 JSON 파일이 업로드되어 있으면 그 벡터를 직접 사용해 3D 좌표를 계산합니다.
4. 그렇지 않고 OpenAI API 키가 있으면 본문을 정리한 뒤 임베딩하고, 캐시에 저장하며, 선택한 투영 방식으로 3D 좌표를 만듭니다.
5. 업로드한 벡터 JSON도 없고 API 키도 없거나, 임베딩 요청이 실패하면 결정적인 구형 레이아웃으로 대체합니다.
6. 구형 레이아웃 모드에서는 경로 해시 기반의 안정적인 순서와 golden-angle 간격을 사용해 노드를 3D 구 내부에 분포시키므로, 새로고침해도 같은 배치를 재현할 수 있지만 의미 기반 배치는 아닙니다.
...
2. OpenAI API 키와 임베딩 모델을 설정하거나, 커스텀 벡터 JSON 파일을 업로드합니다.
3. 둘 다 있으면 업로드한 벡터 JSON이 API 임베딩보다 우선합니다.
4. 리본 아이콘이나 **Open 3D Semantic Graph** 명령으로 뷰를 엽니다.
5. 툴바에서 새로고침, 카메라 리셋, 링크/그리드 토글을 사용할 수 있습니다.
6. 노트를 바로 열려면 노드를 `Shift+클릭`합니다.

## 설정 항목

| 설정 | 설명 | 기본값 |
| --- | --- | --- |
| API Key | OpenAI API 키. 업로드 벡터를 쓸 경우 비워 둘 수 있습니다. 아무 것도 없으면 구형 레이아웃 fallback을 사용합니다. | 비어 있음 |
| Embedding Model | 의미 레이아웃에 사용할 OpenAI 임베딩 모델 | `text-embedding-3-large` |
| Custom Vector JSON | 미리 계산한 벡터 JSON 파일을 업로드합니다. 있으면 API 임베딩 대신 이 파일을 사용합니다. | 비어 있음 |
...
- `entries` 안의 키는 vault 내 노트 경로와 일치해야 합니다.
- 각 `embedding` 값은 숫자 배열이어야 합니다.
- 업로드한 파일은 플러그인 디렉터리의 `uploaded-vectors.json` 으로 저장됩니다.
